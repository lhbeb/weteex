...
function ExternalCheckoutRedirect({ shippingData, provider }: ExternalCheckoutRedirectProps) {
  return (
    <div className="flex flex-col items-center justify-center bg-gradient-to-br from-[#F6F8F5] via-[#EAEFEA] to-[#F6F8F5] px-2 pt-4 min-h-0 sm:pt-16 sm:pb-16">
      <div className="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl p-6 sm:p-10 border border-[#DCE5DE] flex flex-col items-center max-w-md w-full mx-auto transition-all duration-500">
        <div className="flex flex-col items-center mb-4">
          <span className="inline-flex items-center justify-center bg-[#1D2E24] rounded-full p-2 mb-2">
            <Check className="h-7 w-7 text-[#D1A966]" />
          </span>
        </div>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-[#1E2621] tracking-tight mb-2 text-center">
          Address Confirmed
        </h2>
        <p className="text-lg sm:text-xl text-gray-700 mb-4 text-center">
          Your Order Will Be Shipped To The Address Below:
        </p>
        <div className="w-full max-w-xs bg-[#F6F8F5] border border-[#DCE5DE] rounded-2xl shadow p-5 mb-4 flex flex-col gap-2">
          <div className="flex items-center gap-2 mb-1">
            <MapPin className="h-5 w-5 text-[#1D2E24]" />
            <span className="font-semibold text-[#1D2E24] text-base">Confirmed Delivery Address</span>
          </div>
          ...
          {shippingData.email && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-[#1D2E24] text-base">{shippingData.email}</span>
            </div>
          )}
        </div>
        <div className="flex items-center gap-2 text-xs text-gray-500 mb-6">
          <span className="inline-flex items-center justify-center bg-gray-100 rounded-full p-1">
            <svg className="h-4 w-4 text-[#1D2E24]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col items-center gap-2 mt-2 mb-6">
          <div className="w-10 h-10 border-4 border-[#1D2E24]/30 border-t-[#1D2E24] rounded-full animate-spin mb-2" />
