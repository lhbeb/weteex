...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const baseUrl = 'https://weteextees.com';
  const now = new Date();
  ...
  const staticRoutes = [
    ...
    { path: '/return-policy', changeFrequency: 'monthly' as const, priority: 0.5 },
    { path: '/local-pickup', changeFrequency: 'monthly' as const, priority: 0.4 },
    { path: '/track', changeFrequency: 'monthly' as const, priority: 0.4 },
    ...
    { path: '/widerrufsrecht', changeFrequency: 'yearly' as const, priority: 0.4 },
  ];
